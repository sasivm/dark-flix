...
public interface MoviesDBAPIInterface {
    ...
    @GET("{type}/{curr_state}")
    Observable<ResponseBody> getTopCustomMovies(@Path("type") String type, @Path("curr_state") String state, @Query("with_original_language") String language, @Query("region") String region, @Query("api_key") String name);
